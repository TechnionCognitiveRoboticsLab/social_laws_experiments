...
from up_social_laws.social_law import *
...
from up_fast_forward import FastForwardPDDLPlanner
...
def convert_to_ma_and_add_waitfor(problem):
    d = DuplicationsSingleAgentToMultiAgentConverter([problem.user_type("hand")])
    res = d.compile(problem)
    ma_problem = res.problem


    sl = SocialLaw()
    for agent in ma_problem.agents:
        sl.add_waitfor_annotation(agent.name, "grasp", "ontable", ("c"))
    res2 = sl.compile(ma_problem)
    maw_problem = res2.problem
    return maw_problem

def check_robustness(args):
    logging.info("Job pool started job: %s", args)
    domain, problem_name = args
    reader = PDDLReader()
    try:
    ...
    try:
        ma_problem = convert_to_ma_and_add_waitfor(problem)        
    except Exception as e:
    ...
    try:        
        ff_planner = FastForwardPDDLPlanner()
        slrc = SocialLawRobustnessChecker(planner=ff_planner)
